export module Examples:point_repulsion;
...
export void Example::point_repulsion()
{//random
	...
	auto f = [&]()
		{
			...
			app.ResizeVerticies(5 * 3 * 3 * points_count);
			app.ResizeColors(5 * 3 * 3 * points_count);
	...
	do {
		...
		//app.BindBuffers(5 * 3 * 3 * points_count);
		app.BindBuffers();
...
void Create2DCircle_Direct(Point p, Application& app, int begin)
{
	...
	for (int i = 0; i < p.draw_segments; ++i)
	{
		...
		app.VerticiesSetAt(begin + i * 9 + 0, (float)p.x);
		app.VerticiesSetAt(begin + i * 9 + 1, (float)p.y);
		app.VerticiesSetAt(begin + i * 9 + 2, 0.0f);
		...
		app.VerticiesSetAt(begin + i * 9 + 3, (float)p.x + p.radius * cos(pi * (float)i * 2 / p.draw_segments));
		app.VerticiesSetAt(begin + i * 9 + 4, (float)p.y + p.radius * sin(pi * (float)i * 2 / p.draw_segments));
		app.VerticiesSetAt(begin + i * 9 + 5, 0.0f);
		...
		app.VerticiesSetAt(begin + i * 9 + 6, (float)p.x + p.radius * cos(pi * (float)(i + 1) * 2 / p.draw_segments));
		app.VerticiesSetAt(begin + i * 9 + 7, (float)p.y + p.radius * sin(pi * (float)(i + 1) * 2 / p.draw_segments));
		app.VerticiesSetAt(begin + i * 9 + 8, 0.0f);
	}
}
...
void Create2DCircleColor_Direct(Point p, Application& app, int begin)
{
	for (int i = 0; i < p.draw_segments * 9; i += 3)
	{
		...
		app.ColorsSetAt(begin + i,		(float)p.r);
		app.ColorsSetAt(begin + i + 1,	(float)p.g);
		app.ColorsSetAt(begin + i + 2,	(float)p.b);
	}
}
